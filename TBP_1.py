    ...
    def run(self):
        ...
        while True:
            ...
            for t in tracks:
                ...
                # ❌ ETA removed
                label = f"ID:{pid} W:{wait_t}s"

                # 🎨 Person color based on zone (light shades)
                if in_q:
                    color = (255,150,100)   # light blue
                elif in_s:
            ...
            # 🎨 ROI colors (Royal theme)
            cv2.rectangle(frame,tuple(self.cfg.queue_roi[:2]),tuple(self.cfg.queue_roi[2:]),(139,0,0),5)   # Royal dark blue
            cv2.rectangle(frame,tuple(self.cfg.service_roi[:2]),tuple(self.cfg.service_roi[2:]),(0,0,139),5) # Royal dark red

            self.draw_info(frame, queue_count, wait_global, status, color_s, service_count)

            # 🧾 Controls info (bottom)
            cv2.putText(frame,
                "Queue: WASD move | Z/X width | C/V height   ||   Service: Arrows move | I/K height | J/L width",
                (10, 520), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,255,255), 1)
